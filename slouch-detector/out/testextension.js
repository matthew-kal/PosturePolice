...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('PosturePolice.startApp', async () => {
        const backendPath = path.join(context.extensionPath, 'backend', 'api.js');
        const nodePath = 'node'; // 👈 Use Node.js instead of Python
        // 🔹 Start Node backend
        nodeProcess = (0, child_process_1.spawn)(nodePath, [backendPath], {
        ...
        nodeProcess.on('close', (code) => console.log(`Node server exited with code ${code}`));
        // 🔹 Wait for backend to be ready (poll until reachable)
        const maxRetries = 20;
        let tries = 0;
        let serverReady = false;
        while (!serverReady && tries < maxRetries) {
            try {
                await axios_1.default.get(`http://localhost:${backendPort}`);
                serverReady = true;
            }
            catch {
